    ...
    describe "parsing" do
      ...
      context "valid PGN" do
        it "returns true and doesnt have errors" do
          pgn = Pgn.new(:text => valid_pgn_1)

          expect(pgn.save).to eq(true)
          expect(pgn.errors.any?).to eq(false)
        end

        it "saves game with valid pgns" do
          pgn_1 = Pgn.new(:text => valid_pgn_1)
          pgn_2 = Pgn.new(:text => valid_pgn_2)

          expect(pgn_1.save).to eq(true)
          expect(pgn_2.save).to eq(true)

          game_1 = Game.first
          expect(game_1.white).to eq("Mikhail Tal")
          expect(game_1.black).to eq("Bent Larsen")
          expect(game_1.result).to eq("1-0")
          expect(game_1.pgn).to eq(valid_pgn_1)

          game_2 = Game.last
          expect(game_2.white).to eq("Mikhail Tal")
          expect(game_2.black).to eq("Bent Larsen")
          expect(game_2.result).to eq("1-0")
          expect(game_2.pgn).to eq(valid_pgn_2)
        end
